...
public class King extends Pieces {
    ...
    private void moveUpRight(int row, int col){
        if (!Pieces.isOutOfBoard(row - 1, col + 1) && Board.game_board[row - 1][col + 1] == null) {
    ...
    private void takeDownLeft(int row, int col){
        if (!Pieces.isOutOfBoard(row + 1, col - 1)) {
    ...
    private void moveRight(int row, int col){
        if (!Pieces.isOutOfBoard(row, col + 1)) {
    ...
    void avoidOppKing(List<Coordinates<Integer, Integer>> filteredList) {
        Coordinates opp_king_position = Pieces.findFigure(King.class, this.color.oppositeColor());
    ...
    public void updateZone() {
        ...
        for (int dr = -1; dr <= 1; dr++) {
            for (int dc = -1; dc <= 1; dc++) {
                ...
                if (!Pieces.isOutOfBoard(newRow, newCol)) {
    ...
    public int calculateMoveStrength(Coordinates<Integer, Integer> move) {
        ...
        if(!(move == null)) {
            ...
            if(Math.abs(this.position.getY() - move.getY()) == 2){
                strength += 500;
            }
        }
        return strength;
