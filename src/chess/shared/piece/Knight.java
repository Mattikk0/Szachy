...
import java.util.ArrayList;
 ...
 public class Knight extends Pieces {
    ...
    void moveUpRight(int row, int col){
        if (!Pieces.isOutOfBoard(row - 2, col + 1) && Board.game_board[row - 2][col + 1] == null) {
    ...
    void moveRightUp(int row, int col){
        if (!Pieces.isOutOfBoard(row - 1, col + 2) && Board.game_board[row - 1][col + 2] == null) {
    ...
    void takeLeftUp(int row, int col){
        if (!Pieces.isOutOfBoard(row -1, col - 2)) {
    ...
    void takeDownLeft(int row, int col){
        if (!Pieces.isOutOfBoard(row +2, col - 1)) {
    ...
    public void drawPiece(PieceColor color, Label label) {
     ...
     @Override
     boolean isChecking() {
        Coordinates king_position = Pieces.findFigure(King.class, this.color.oppositeColor());
